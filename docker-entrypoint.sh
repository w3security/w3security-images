...
if ! [ -z "${COMMAND}" ]; then
    ...
    # Python
    # w3security requires Python to have downloaded the dependencies before running
    ...
    # Maven
    # w3security requires Maven to have downloaded the dependencies before running
    ...
    # Go dep
    # w3security requires dep to be installed
...
if [ $exit_code -ne 0 ]; then
    printf '%s\n' "$out"
    exit $exit_code
# By default we don't output any of the commands needed to run before w3security
...
if [ "$INPUT_COMMAND" = "test" -a "$INPUT_JSON" = "true" ]; then
    JSON_OUTPUT="--json-file-output=w3security.json"
fi
...
if [ "$INPUT_COMMAND" = "test" -a "$INPUT_SARIF" = "true" ]; then
    ...
    if [ "${args/--file}" != "$args" ] || ! [ -z "$INPUT_FILE" ] ; then
        SARIF_OUTPUT="--sarif-file-output=w3security.sarif"
    fi
fi
