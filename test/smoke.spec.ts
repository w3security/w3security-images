...
describe('smoke tests', () => {
  it('can do `w3security version`', async () => {
    const imageName = process.env.IMAGE_TAG;
    const { stdout, stderr } = await runContainer(imageName, 'w3security version');
  ...
  it('Entrypoint adding JSON_OUTPUT', async () => {
    ...
    expect(actual).toEqual('something else --json-file-output=w3security.json');
  });
  ...
  it('Entrypoint adding SARIF_OUTPUT', async () => {
    ...
    expect(actual).toEqual(
      'something else --file=filename --sarif-file-output=w3security.sarif',
    );
  });
});
